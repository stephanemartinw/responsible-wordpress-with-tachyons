...
<noscript>
	...
	<nav class="<?php echo $navigation_args['classes']['navigation']; ?> db" role="navigation">
			<button class="<?php echo $navigation_args['classes']['button']; ?>" aria-controls="main-menu" aria-expanded="false"><?php esc_html_e( 'Menu', 'wistiti' ); ?></button>
			...
			<?php
				wp_nav_menu(array(
				    'theme_location' => 'menu-1',
						'container'=> false,
						'items_wrap' => '<ul id="nojs-main-menu" class="'.$navigation_args['classes']['items']['level'][0]['list'].' db cmzr-navigation-menu" role="menubar">%3$s</ul>',
				    'walker'  => new Wistiti_Walker_Main_Menu($navigation_args) //use wistiti custom walker in functions.php
				));
				?>
	</nav>
